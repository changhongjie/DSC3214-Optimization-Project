...
csvdata = readcsv("A1_data_incumbents.csv", header = true)
data = csvdata[1]
...
fleet = map(Int64, data[1,6])                   # total bus fleet size

alighting = alightdata
journey = ceil(Int, journeytime/intervaltime)   # how many intervals are equivalent to 1 journey
...
for i in 1:(rows-journey+1)
  @constraint(m, sum{x[i+j], j=0:(journey-1)} <= fleet) #every set of j intervals meets fleet constraint
end
# move unsatisfied customers to next intervals
for j in 1:columns
  @constraint(m, tdemand[1,j] == demand[1,j]) #no unsatisfied customers before 1st interval
  for i in 1:rows-1
